                ...
                <>
                    ...
                    >
                        <div className="max-w-[1400px] mx-auto grid grid-cols-4 gap-12">
                            ...
                            <div>
                                <h3 onClick={() => { setSelectedSubCategory('All'); setShowMegaMenu(false); }} className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40 mb-8 cursor-pointer hover:text-white transition-colors">Accessories</h3>
                                <div className="flex flex-col space-y-4">
                                    {['Jewelry', 'Bags', 'Footwear', 'Belts', 'Eyewear'].map(item => (
                                        <button key={item} onClick={() => { setSelectedSubCategory(item); setShowMegaMenu(false); }} className="text-2xl font-black italic tracking-tighter hover:translate-x-4 transition-transform text-left text-white">{item}</button>
                                    ))}
                                </div>
                            </div>
                            <div>
